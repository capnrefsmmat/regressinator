...
Using `diagnose_model()`, we can compare the true residual plots to several where
...
```{r, fig.height=8}
diagnose_model(fit, fn = augment_longer, n = 5) |>
  ggplot(aes(x = .predictor_value, y = .resid)) +
  geom_point() +
  geom_smooth(se = FALSE) +
...
We can again use `diagnose_model()` to see how these compare to the partial
residuals when the model is correctly specified:

```{r, fig.height=8}
diagnose_model(fit, partial_residuals, n = 5) |>
  ggplot(aes(x = predictor_value, y = partial_resid)) +
  geom_point() +
  geom_smooth(se = FALSE) +
  facet_grid(rows = vars(.sample), cols = vars(predictor_name),
