...
Ordinary standardized residuals are not very helpful for noticing the
misspecification:
...
binned_residuals(fit) |>
  ggplot(aes(x = .fitted.mean, y = .resid.mean, size = .n)) +
  geom_point() +
...
binned_residuals(fit, term = "x1") |>
  ggplot(aes(x = x1.mean, y = .resid.mean, size = .n)) +
  geom_point() +
